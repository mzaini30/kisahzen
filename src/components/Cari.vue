...
function mulaiCari() {
  ...
  semuaPostingan.forEach((x) => {
    if (
      !x.innerText.toLowerCase().includes(teksCari.value.toLowerCase().trim())
    ) {
...
function reset() {
  teksCari.value = "";
  document
    .querySelectorAll(".list-postingan")
...
<template>
  <div class="w-full flex gap-3 mb-4">
    ...
    />
    <button class="bg-slate-700 rounded px-4 py-2 text-white" @click="reset">
      reset
    </button>
  </div>
</template>
